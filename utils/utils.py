...
def save_sample_q(model, epoch, arg, num=100, save=True, i=0, video=False):
    ...
    if save:
        tv_utils.save_image(all_images, os.path.join(arg.save_path, 'samples', f'sample-{milestone}.png'), normalize=True, nrow=int(sqrt(num)))
    return all_images


def iddpm_sample_q(model, diffusion, epoch, arg, num=100, save=True, i=0, title=''):
    milestone = str(epoch)
    if i > 0:
        milestone += '-' + str(i)

    all_images = diffusion.p_sample_loop(
        model, (num, 3, arg.img_size, arg.img_size), clip_denoised=True, model_kwargs=None
    )

    if save:
        tv_utils.save_image(all_images, os.path.join(arg.save_path, 'samples', f'sample-{milestone}%s.png' % title), normalize=True, nrow=int(sqrt(num)))
    return all_images
...
def sample_ema_iddpm(ema_model, diffusion, buffer, epoch, arg, title='ema'):
    ema_model.eval()
    q = iddpm_sample_q(ema_model, diffusion, epoch, arg)
    idx_start = 0
    if epoch > 20:
        idx_start = (epoch - 20) % 100 * (100 if arg.dataset != 'stl10' else 50)
    buffer[idx_start:idx_start + 100] = q
